...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const extractedCalories = data.nutrition.nutrients.find(
      (nutrient) => nutrient.name.toLowerCase() === "calories"
    )?.amount;

    const cleanedInstructions = fixSentenceSpacing(
      removeTags(data.instructions)
    );

    const splitInstructions = extractTextToArray(cleanedInstructions);
